...
function obterDadosDoPocessadorTemp(idMaquina) {
  console.log("Processador-ao-Longo-do-Tempo")
  ...
  fetch(`/processador/ultimasCPU/${idMaquina}`, { cache: 'no-store' }).then(function (response) {
      if (response.ok) {
          response.json().then(function (resposta) {
              ...
              plotarGraficoPocessadorTemp(resposta, idMaquina);
...
function plotarGraficoPocessadorTemp(resposta, idMaquina) {
    ...
    let chartProcessadortemp = new Chart(
      document.getElementById(`myChartProceTemp`),
      config
    );
  
    setTimeout(() => atualizarGraficoPocessadorTemp(idMaquina, dados, chartProcessadortemp), 5000);
  }
  
  

function atualizarGraficoPocessadorTemp(idMaquina, dados, chartProcessadortemp) {
        ...
        if (response.ok) {
            response.json().then(function (novoRegistro) {
                ...
                if (novoRegistro[0].data_hora == dados.datasets[0].data.data_hora) {
                    ...
                    chartProcessadortemp.update();
                }

                // Altere aqui o valor em ms se quiser que o gráfico atualize mais rápido ou mais devagar
                proximaAtualizacao = setTimeout(() => atualizarGraficoPocessadorTemp(idMaquina, dados, chartProcessadortemp), 5000);
            });
        } else {
            console.error('Nenhum dado encontrado ou erro na API');
            // Altere aqui o valor em ms se quiser que o gráfico atualize mais rápido ou mais devagar
...
function limparPocessadorTemp(){
    let chartProcessadortemp = new Chart(
        document.getElementById(`myChartProceTemp`),
    );

    chartProcessadortemp.clear()
}